...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/14 15:46:49 by vasferre          #+#    #+#             */
/*   Updated: 2022/11/15 15:14:37 by vasferre         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*get_line(char **buffer_backup)
{
	...
	while ((*buffer_backup)[i] != '\0' && (*buffer_backup)[i] != '\n')
		i++;
	line = ft_substr(*buffer_backup, 0, i + is_end_line(*buffer_backup));
	if (!line)
	{
		free(line);
		return (NULL);
...
static int	read_file(int fd, char **buffer, char **buffer_backup, char **line)
{
	...
	bytes_read = 1;
	while (is_end_line(*buffer_backup) != 1 && bytes_read > 0)
	{
